...
if(instance_exists(oPlayer)){
	if(room != rmGameOver && room != rmPong){
		...
		if(index_multi == 1) draw_txt_centered(_icon_x, _icon_y + _text_spacing, instance_number(oBall), fntDefault, c_white);
	
		draw_sprite(sIcon_wide, index_wide, _icon_x + 64, _icon_y);
		if(index_wide == 1) draw_txt_centered(_icon_x + 64, _icon_y + _text_spacing, ceil(oPlayer.alarm[0] / 60), fntDefault, c_white);
	
		draw_sprite(sIcon_narrow, index_narrow, _icon_x + 128, _icon_y);
		if(index_narrow == 1) draw_txt_centered(_icon_x + 128, _icon_y + _text_spacing, ceil(oPlayer.alarm[0] / 60), fntDefault, c_white);
	
		draw_sprite(sIcon_pierce, index_pierce, _icon_x, _icon_y - 64);
		if(index_pierce == 1) draw_txt_centered(_icon_x, _icon_y - 64 + _text_spacing, ceil(Director.alarm[0] / 60), fntDefault, c_white);
	
		draw_sprite(sIcon_spread, index_spread, _icon_x + 64, _icon_y - 64);
		if(index_spread == 1) draw_txt_centered(_icon_x + 64, _icon_y - 64 + _text_spacing, ceil(oPlayer.alarm[1] / 60), fntDefault, c_white);
	
		draw_sprite(sIcon_shield, index_shield, _icon_x + 128, _icon_y - 64);
		if(index_shield == 1) draw_txt_centered(_icon_x + 128, _icon_y - 64 + _text_spacing, oPlayerShield.life, fntDefault, c_white);
	
		draw_sprite(sIcon_speed, index_speed, _icon_x, _icon_y - 128);
		if(index_speed == 1) draw_txt_centered(_icon_x, _icon_y - 128 + _text_spacing, ceil(oPlayer.alarm[2] / 60), fntDefault, c_white);
	}
}